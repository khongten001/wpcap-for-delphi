...

{
 ...
 TDHCPHeader = packed record
  ...
  TDCHPAuthentication  = packed record
    Protocol          : Byte;       
    Algorithm         : Byte;        
    RDM               : Byte;      
  ...
  PTDCHPAuthentication = ^TDCHPAuthentication; 
...
var LUDPPayLoad         : PByte;
    ...
    LWordValue          : Word;        
    I                   : Integer;
    LDCHPAuthentication : PTDCHPAuthentication;
begin
  Result := False;
  ...
  LHeaderDCHP := PTDHCPHeader(LUDPPayLoad);

 {  op 1  Message op code / message type.
       1 = BOOTREQUEST, 2 = BOOTREPLY}  
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Message type:', ifthen(LHeaderDCHP.OpCode=1,'Boot request','Boot reply'), @LHeaderDCHP.OpCode,SizeOf(LHeaderDCHP.OpCode)));

 {  htype  1  Hardware address type, see ARP section in "Assigned
              Numbers" RFC; e.g., '1' = 10mb ethernet.  }   
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Hardware type:', LHeaderDCHP.HType, @LHeaderDCHP.HType,SizeOf(LHeaderDCHP.HType)));
  
 {  hlen   1  Hardware address length (e.g.  '6' for 10mb
              ethernet).}
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Hardware address length:', LHeaderDCHP.HLen, @LHeaderDCHP.HLen,SizeOf(LHeaderDCHP.HLen)));  

 {  hops   1  Client sets to zero, optionally used by relay agents
              when booting via a relay agent.}
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Hops:', LHeaderDCHP.Hops, @LHeaderDCHP.Hops,SizeOf(LHeaderDCHP.Hops)));    

 {  xid    4  Transaction ID, a random number chosen by the
              client, used by the client and server to associate
              messages and responses between a client and a
              server.}
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Transaction ID:', LongWordToString(LHeaderDCHP.TransactionID), @LHeaderDCHP.TransactionID,SizeOf(LHeaderDCHP.TransactionID)));    

  { secs   2  Filled in by client, seconds elapsed since client
              began address acquisition or renewal process. }
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Seconds Elapsed:', wpcapntohs(LHeaderDCHP.SecondsElapsed), @LHeaderDCHP.SecondsElapsed,SizeOf(LHeaderDCHP.SecondsElapsed)));      

  { flags   2  Flags (see figure 2).}
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Bootp flags:', ByteToBinaryString( LHeaderDCHP.Flags), @LHeaderDCHP.Flags,SizeOf(LHeaderDCHP.Flags)));        
  ...
  AListDetail.Add(AddHeaderInfo(aStartLevel+2, 'Broadcast:', GetBitValue( LHeaderDCHP.Flags,1), @LHeaderDCHP.Flags,SizeOf(LHeaderDCHP.Flags)));    

  { ciaddr  4  Client IP address; only filled in if client is in
               BOUND, RENEW or REBINDING state and can respond
               to ARP requests.}  
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Client IP address:', intToIPV4(LHeaderDCHP.ClientIP), @LHeaderDCHP.ClientIP,SizeOf(LHeaderDCHP.ClientIP)));          

  { yiaddr  4  'your' (client) IP address.}
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Your (client) IP address:', intToIPV4(LHeaderDCHP.YourIP), @LHeaderDCHP.YourIP,SizeOf(LHeaderDCHP.YourIP)));          

  { siaddr  4  IP address of next server to use in bootstrap;
               returned in DHCPOFFER, DHCPACK by server.      }
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Next server IP address:', intToIPV4(LHeaderDCHP.ServerIP), @LHeaderDCHP.ServerIP,SizeOf(LHeaderDCHP.ServerIP)));            

 { giaddr   4  Relay agent IP address, used in booting via a
               relay agent.}
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Relay Agent IP address:', intToIPV4(LHeaderDCHP.RelayAgentIP), @LHeaderDCHP.RelayAgentIP,SizeOf(LHeaderDCHP.RelayAgentIP)));          

  {chaddr   16  Client hardware address.  }
  SetLength(LBytesTmp,LHeaderDCHP.HLen);
  Move(LHeaderDCHP.ClientMAC[0], LBytesTmp[0], LHeaderDCHP.HLen);   
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Client MAC address:', MACAddressToString(LBytesTmp), @LBytesTmp,SizeOf(LBytesTmp)));          
  SetLength(LBytesTmp,SizeOf(LHeaderDCHP.ClientMAC)- LHeaderDCHP.HLen);
  Move(LHeaderDCHP.ClientMAC[LHeaderDCHP.HLen], LBytesTmp[0], SizeOf(LHeaderDCHP.ClientMAC)- LHeaderDCHP.HLen);   
  ...
  SetLength(LBytesTmp, SizeOf(LHeaderDCHP.ServerName));
  Move(LHeaderDCHP.ServerName[0], LBytesTmp[0], SizeOf(LHeaderDCHP.ServerName));  
 ...
 {  file    128  Boot file name, null terminated string; "generic"
  ...
  SetLength(LBytesTmp, SizeOf(LHeaderDCHP.BootFileName));
  Move(LHeaderDCHP.BootFileName[0], LBytesTmp[0], SizeOf(LHeaderDCHP.BootFileName));  
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Boot FileName:', BytesToStringRaw(LBytesTmp), @LBytesTmp,SizeOf(LBytesTmp)));      
    
  
  AListDetail.Add(AddHeaderInfo(aStartLevel+1, 'Magic Cookie:', wpcapntohl(LHeaderDCHP.MagicCookie), @LHeaderDCHP.MagicCookie,SizeOf(LHeaderDCHP.MagicCookie)));
    ...
    begin
      ...
      case LOption of
        ...
        OPTION_AUTHENTICATION:
          begin
            LDCHPAuthentication := PTDCHPAuthentication(LUDPPayLoad + LCurrentPos);
            AListDetail.Add(AddHeaderInfo(aStartLevel+2,'Protocol:', DecimalToAuthenticationSuboption(LDCHPAuthentication.Protocol), @LDCHPAuthentication.Protocol,sizeOf(LDCHPAuthentication.Protocol)));
            AListDetail.Add(AddHeaderInfo(aStartLevel+2,'Algorithm:',ifthen(LDCHPAuthentication.Algorithm=1,'HMAC-SHA1 keyed hash','Reserved'), @LDCHPAuthentication.Algorithm,sizeOf(LDCHPAuthentication.Algorithm)));
            AListDetail.Add(AddHeaderInfo(aStartLevel+2,'Replay Detection Method:',ifthen(LDCHPAuthentication.RDM=0,'use of a monotonically increasing counter value','Reserved'), @LDCHPAuthentication.RDM,sizeOf(LDCHPAuthentication.RDM)));
            AListDetail.Add(AddHeaderInfo(aStartLevel+2,'Replay Detection value:', ntohl(LDCHPAuthentication.ReplayDetection), @LDCHPAuthentication.ReplayDetection,sizeOf(LDCHPAuthentication.ReplayDetection)));                                                                 
            AListDetail.Add(AddHeaderInfo(aStartLevel+2,'Secret ID:', wpcapntohl(LDCHPAuthentication.SecretID), @LDCHPAuthentication.SecretID,sizeOf(LDCHPAuthentication.SecretID)));                 
            SetLength(LBytesTmp, SizeOf(LDCHPAuthentication.AuthenticationInfo));
            Move(LDCHPAuthentication.AuthenticationInfo[0], LBytesTmp[0], SizeOf(LDCHPAuthentication.AuthenticationInfo));              
            AListDetail.Add(AddHeaderInfo(aStartLevel+2,'HMAC MD5 Hash:',ToHex(LBytesTmp), @LDCHPAuthentication.AuthenticationInfo,sizeOf(LDCHPAuthentication.AuthenticationInfo)));                 
            inc(LCurrentPos,LLen);
          end;
